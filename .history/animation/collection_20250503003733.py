...
class GifCollection[Content]:
    ...
    def compare_priority(self, anim_index1, gif_index2):
        if anim_index1 == gif_index2: return False
        return self.get_priority(anim_index1) <= self.get_priority(gif_index2)

    def decide(self, state, start=None, repeat=None):
        self.__state == 5
        # if x: ic(str(self), self.get_priority(1), self.get_priority(5), )
        gif = self.get_animation()
