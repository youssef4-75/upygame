...
pygame surface or a list-like of pygame surfaces""")
    ...
    def __add__(self, other):
        if isinstance(other, Surface):            return Animation([other] + list(self.SL))
        if isinstance(other, Animation): return Animation(list(other.SL) + list(self.SL))
        if isinstance(other, Iterable): return Animation(list(other) + list(self.SL))
        raise TypeError("Animation.__radd__: other must be a Surface, Surface list or an Animation")
