...
class AnimationSet:
    ...
    def get_from_directory(self, directory: str, size: tuple=None, separator: str="x"):
        """
        ...
        for d in listdir(directory):
            if not d.startswith(".") and not d.endswith(".py"):
                try:
                    key, priority = map(int, d.split(separator))
                except ValueError:
                    print(f"AnimationSet.get_from_directory: {d} is not in the form key{separator}priority")
                    continue
                path = f"{directory}/{d}"
                self.add(key, Animation.from_directory(path, size), priority)
        ...
