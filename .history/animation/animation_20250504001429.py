...
pygame surface or a list-like of pygame surfaces""")
    ...
    def generate(self, frame_speed):
        """
        generate the next frame of the animation, if ended, restart
        """
    ...
    def update(self, start=None, repeat=None):
        """
        reset the starting point and the endpoint of an animation, start is the """
        if start: self.__start = RATIO * start
        if repeat:self.__repeat = RATIO * min(repeat, len(self.SL) - 1)
