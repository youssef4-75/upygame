...
class AnimationSet:
    ...
    @classmethod
    def new(cls, surf: Surface, frame_speed):
        return cls({0: Animation([surf])}, {}, frame_speed)
    
    def add_sample_color(self, 
                key, color: tuple[int, int, int], 
                size: tuple[int, int]=(100, 100)):
        transparent_surface = Surface(size, SRCALPHA)
        transparent_surface.fill(color)  
        self.__animations[key] = transparent_surface
    
    def add(self, key: int, Gif: Animation, priority: int):
    ...
    def decide(self, state=None, start=None, repeat=None):
        ...
        """
        if state is None: state = self.__state
