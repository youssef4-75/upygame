...
class Board:
    ...
    def activate_peace(self, mouse_pos: tuple[Column_X, Row_Y]):
        """
        select the cell to 
        """
        if self.moving_peaces is not None: raise MoveCollisionError()
        row, column = self.translate_mouse_click(mouse_pos)
        for peace in self.peaces:
            if peace.row == row and peace.column == column:
                if peace.side != self.turn: raise
