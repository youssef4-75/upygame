...
class Window:
    ...
    @dispatch(object, pg.Surface, object)
    def blit(self, screen: pg.Surface, pos: object):
        if isinstance(pos, tuple) or isinstance(pos, list):
            pos = pg.Rect(pos[0], pos[1], screen.get_width(), screen.get_height())
        # self.screen.blit(screen, pos)
        return "bliting a surface with a rect"

    """
    @dispatch(object, AnimationSet, Union[tuple, pg.Rect])
    def blit(self, screen: AnimationSet, pos: tuple|pg.Rect):
        # self.screen.blit(screen, pos)
        return "bliting a collection"     
    """
