...
class Window:
    ...
    @dispatch(Board)
    def blit(self, surface: Board, *, limits=None):
        for i in range(surface.row):
            for j in range(surface.column):
                rect, color = surface.get_cell(i, j)
                surf = pg.Surface((rect.width, rect.height), pg.SRCALPHA)
                surf.fill(color)
                self.screen.blit(surf, rect)
        if limits is None: return
        for i in range(surface.row):
            pg.draw.line(self.screen, limits, 
                    (surface.left, surface.top+ i*surface.cell_height),
                    (surface.right, i*surface.cell_height))
